...
def create_response(
    data: Any, status_code: int = 200, metadata: Dict[str, Any] = None
) -> Tuple[Dict, int]:
...
def create_user():
    ...
    return create_response(
        data=user_data, status_code=201, metadata={"user_id": user.id}
    )
...
def get_user(user_id: int):
    ...
    if not user:
        raise AppError(
            message=f"User {user_id} not found", code="USER_NOT_FOUND", status_code=404
        )
